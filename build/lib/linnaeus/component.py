...
class ComponentImage(object):
    def __init__(self, path=None, cache=None, base_url='{0}'):
        ...
        if cache is not None:
            self._dominant, self._dominant_nongrey = cache[self.id]
        else:
            self._dominant = None
...
class ComponentCache(object):
    ...
    def __getitem__(self, item):
        return self._items.get(item, (None, None))

    def cache(self, component: ComponentImage):
        self._items[component.id] = (component.dominant, component.dominant_nongrey)
